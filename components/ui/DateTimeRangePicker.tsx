...
export default function DateTimeRangePicker({ value, onChange, className = '' }: DateTimeRangePickerProps) {
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (nextOpen) {
      ...
      if (currentRange?.from) {
        setStartTime({
          hours: currentRange.from.getHours(),
          minutes: currentRange.from.getMinutes(),
        });
      }
      if (currentRange?.to) {
        setEndTime({
          hours: currentRange.to.getHours(),
          minutes: currentRange.to.getMinutes(),
        });
      }
    }
  };
  ...
  const handlePresetClick = (preset: (typeof PRESET_RANGES)[0]) => {
    ...
    setStartTime({
      hours: presetRange.from.getHours(),
      minutes: presetRange.from.getMinutes(),
    });
    setEndTime({
      hours: presetRange.to.getHours(),
      minutes: presetRange.to.getMinutes(),
    });
  };
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          className={`w-full cursor-pointer rounded-md border border-[var(--color-border)] bg-[var(--color-input)] px-3 py-2 text-left text-sm text-[var(--color-foreground)] shadow-sm hover:border-[var(--color-ring)] focus:border-[var(--color-ring)] focus:outline-none focus:ring-[var(--color-ring)] ${className}`.trim()}
        >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-[900px] -translate-x-1/2 -translate-y-1/2 px-4 py-6 focus:outline-none">
                ...
                <Box className="flex-1">
                  ...
                  <Flex gap="2" align="center" className="mb-2">
                    <Input
                      ...
                      value={
                        tempRange?.from
                          ? format(tempRange.from, "yyyy-MM-dd'T'HH:mm")
                          : ''
                      }
                ...
                <Box className="flex-1">
                  ...
                  <Flex gap="2" align="center" className="mb-2">
                    <Input
                      ...
                      value={
                        tempRange?.to
                          ? format(tempRange.to, "yyyy-MM-dd'T'HH:mm")
                          : ''
                      }
                ...
                <Box className="flex-1">
                  <DayPicker
                    ...
                    classNames={{
                      ...
                      head_row: 'flex',
                      head_cell:
                        'text-[var(--color-muted-foreground)] rounded-md w-9 font-normal text-[0.8rem]',
